...
func TestOutput_Status(t *testing.T) {
	for _, code := range []int{200, 404, 500} {
		...
		out := cmdio.NewOutputWriter(&buf, coretypes.Colors{}, false, false)
		out.Write(makeResp(code, nil, nil))
		got := buf.String()
...
func TestOutput_Elapsed(t *testing.T) {
	var buf bytes.Buffer
	out := cmdio.NewOutputWriter(&buf, coretypes.Colors{}, false, false)
	out.Write(makeResp(200, nil, nil))
	got := buf.String()
...
func TestOutput_BodyRaw(t *testing.T) {
	...
	var buf bytes.Buffer
	out := cmdio.NewOutputWriter(&buf, coretypes.Colors{}, false, false)
	out.Write(makeResp(200, body, nil))
	got := buf.String()
...
func TestOutput_BodyPretty(t *testing.T) {
	body := []byte(`{"name":"alex"}`)
	var buf bytes.Buffer
	out := cmdio.NewOutputWriter(&buf, coretypes.Colors{}, true, false)
	out.Write(makeResp(200, body, nil))
...
func TestOutput_BodyEmpty(t *testing.T) {
	var buf bytes.Buffer
	out := cmdio.NewOutputWriter(&buf, coretypes.Colors{}, false, false)
	out.Write(makeResp(204, nil, nil))
...
func TestOutput_HeadersVerbose(t *testing.T) {
	...
	var buf bytes.Buffer
	out := cmdio.NewOutputWriter(&buf, coretypes.Colors{}, false, true)
	out.Write(makeResp(200, nil, headers))
	got := buf.String()
